...
const fetchData = async () => {
  try {
    let result = await axios.get(`https://api.potterdb.com/v1/books?page[size]=7&page[number]=${pageNumber.value}`);
  ...
  <div class="container">
    ...
    <div class="books-list">
      <div v-for="item in list" :key="item.id" class="books-item">
        <img :src="item.attributes.cover" alt="Image du livre" />
        <div class="potion-details">
          ...
          <p v-if="item.attributes.release_date"><span>Date de sorti : </span>{{ item.attributes.release_date }}</p>
